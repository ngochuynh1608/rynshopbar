...
class ControllerInformationNews extends Controller {
	public function index() {
		...
		$total = $this->model_extension_news->getTotalNews($filter_data);
		$pagination = new Pagination();
		$pagination->total = $total;
		$pagination->page = $page;
		$pagination->limit = 10;
		$pagination->url = $this->url->link('information/news&category='.$category, 'page={page}');
		if($total==0 || $total==" ")
        {
            $data['pagination'] = "";
        }
		...
		foreach ($all_news as $news) {
			$data['all_news'][] = array (
                ...
                //'image' => HTTP_SERVER . 'image/' . $news['image'],
        ...
        if($category==2)
        {
    		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/information/recipes.tpl')) {
    			$this->response->setOutput($this->load->view($this->config->get('config_template') . '/template/information/recipes.tpl', $data));
    		} else {
    			$this->response->setOutput($this->load->view('default/template/information/recipes.tpl', $data));
