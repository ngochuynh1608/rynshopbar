<?php echo $header; ?>
<main>
<script type="text/javascript" src="catalog/view/theme/rynshopbar/javascript/jquery/jquery-2.1.1.min.js"></script>
    <div class="main-part">        
      ...
      <section class="default-section shop-page">
        <div class="container">
          <div class="row">
          <div class="error"></div>
          <div class=" box checkout_form mt100">
              <div class="col-md-5 register_block">
            <div class="row">
              ...
              <div class="your_order">
                ...
                <div class="payment clearfix">
                    <?php if ($payment) echo $payment; else {?>
                              <div class="pull-right">
                       <input type="button" class="btn btn-checkout" data-loading-text="<?php if (isset($text_loading)) echo $text_loading;else echo 'loading ...' ?>" id="button-register" value="<?php echo $heading_title;?>">
                    </div>
                              <?php }?></div>
                </div>
                </div>
...
$(document).delegate('#button-register', 'click', function() 
{
  ...
  data += '&_shipping_method='+ jQuery('.checkout_form input[name=\'shipping_method\']:checked').prop('title') + '&_payment_method=' + jQuery('.checkout_form input[name=\'payment_method\']:checked').prop('title');
    ...
    $.ajax({
        ...
        dataType: 'json',
        beforeSend: function() {
      $('#button-register').html('loading');
    },  
        complete: function() {
            $('#button-register').html('reset');
        },          
        success: function(json) {
            $('.alert, .text-danger').remove();
...
{
    var value=$('select[name=\'zone_id\']').val();
    if(value=='3767' || value=='3780' || value=='3776')
    {
        jQuery("input[name=\'shipping_method\']:last").attr('checked', 'checked').prop('checked', true).click();
    }
    else
    {
...
$('select[name=\'country_id\'],  select[name=\'shipping_country_id\'], select[name=\'shipping_zone_id\'], input[type=\'radio\'][name=\'payment_address\'], input[type=\'radio\'][name=\'shipping_address\']').on('change', function() 
{
